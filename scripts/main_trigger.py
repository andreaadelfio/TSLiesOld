...
from modules.background import RNNPredictor, FFNNPredictor, PBNNPredictor, BNNPredictor
from modules.utils import Data, File
from modules.plotter import Plotter
from modules.trigger import Trigger

from scripts.main_config import y_cols, y_cols_raw, y_pred_cols, x_cols, x_cols_excluded, units, y_smooth_cols, latex_y_cols, thresholds
import pandas as pd


def run_trigger_ffnn(inputs_outputs_df, y_cols, y_cols_raw, y_cols_pred, x_cols, file):
    '''Runs the model'''
    y_pred = None
...
def run_trigger_pbnn(inputs_outputs_df, y_cols, y_cols_raw, y_cols_pred, x_cols):
    '''Runs the model'''
    nn = PBNNPredictor(inputs_outputs_df, y_cols, x_cols, y_cols_raw, y_cols_pred, y_smooth_cols)
...
def run_trigger_bnn(inputs_outputs_df, y_cols, y_cols_raw, y_cols_pred, x_cols, file):
    '''Runs the model'''
    nn = BNNPredictor(inputs_outputs_df, y_cols, x_cols, y_cols_raw, y_pred_cols, y_smooth_cols, latex_y_cols, False)
    nn.set_model(model_path='results/2025-05-27/background_prediction/1532/BNNPredictor/0/model.keras', compile=False)
    nn.load_scalers()
    start, end = 0, -1
    batch_size = len(inputs_outputs_df)
    for i in range(0, len(inputs_outputs_df), batch_size):
...
def run_trigger_mean(inputs_outputs_df, y_cols, y_cols_raw, y_cols_pred, x_cols, file):
    '''Runs the model'''
    tiles_df = inputs_outputs_df.copy()
    stats = []
    for face in y_cols:
    ...
    Trigger(tiles_df, y_cols, y_cols_pred, y_cols, units, latex_y_cols).run(thresholds, type='FOCUS', save_anomalies_plots=True, support_vars=support_vars, file=file)

if __name__ == '__main__':
    x_cols = [col for col in x_cols if col not in x_cols_excluded]
    for i in range(860, 1000):
        inputs_outputs_df = File().read_dfs_from_weekly_pk_folder(start=i, stop=i)
        ...
        run_trigger_mean(inputs_outputs_df, y_cols, y_cols_raw, y_pred_cols, x_cols, i)
        # run_trigger_bnn(inputs_outputs_df, y_cols, y_cols_raw, y_pred_cols, x_cols, i)
        # run_trigger_ffnn(inputs_outputs_df, y_cols, y_cols_raw, y_pred_cols, x_cols, i)
        gc.collect()