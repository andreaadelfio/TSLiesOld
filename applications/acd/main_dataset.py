'''
ACD-specific dataset creation script

This script demonstrates the use of the generic time series framework 
for creating ACD datasets combining spacecraft, solar, and detector data.
...
def get_tiles_signal_df():
    ...
    print('Catalog...', end='')
    cr = DatasetReader(h_names=y_cols_raw, start=0, end=-1)
...
if __name__ == '__main__':
    ...
    inputs_outputs_df = File().read_dfs_from_weekly_pk_folder(start=836, stop=837, cols_list=y_cols + x_cols + y_smooth_cols + ['datetime'], y_cols=y_cols, resample_skip=1)
    inputs_outputs_df = Data.get_masked_dataframe(data=inputs_outputs_df,
                                                  start='2024-06-09 11:30:00',
                                                  stop='2024-06-09 12:05:00')
