...
class LimeTabularExplainer(object):
    ...
    def explain_instance(self,
        ...
        # for regression, the output should be a one-dimensional array of predictions
            ...
            try:
                if len(yss.shape) != 1 and len(yss[0].shape) == 1:
                    yss = np.array([v[mylabel] for v in yss])
        ...
        if self.mode == "regression":
            ret_exp.intercept[1 + mylabel] = ret_exp.intercept[0 + mylabel]
            ret_exp.local_exp[1 + mylabel] = [x for x in ret_exp.local_exp[0 + mylabel]]
            ret_exp.local_exp[0 + mylabel] = [(i, -1 * j) for i, j in ret_exp.local_exp[1 + mylabel]]
